...
- **Logo de marque** (`BrandPickerModal`, calque flottant) : **tout** le set `simple-icons` (~3000 marques, `lib/brandLogos.ts` → `BRAND_LOGOS`/`BRAND_MAP` via `import * as`, couleur officielle ; recherche capée à 180 à l'affichage). Stocké sur le calque `brand` (`brandSlug`).
- **Glyphe de pilule** : une pilule affiche par défaut une flèche d'accent, ou une **icône/emoji custom** (mêmes champs `iconName`/`iconEmoji` que le calque icône, via le picker), ou **rien** (`noGlyph`). Réglé par le segment ▶/icône/∅ de la ligne du calque.
