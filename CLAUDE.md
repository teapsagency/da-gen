...
- **Glyphe de pilule** : **rien** (défaut) ou une **icône/emoji custom** (mêmes champs `iconName`/`iconEmoji` que le calque icône, via le picker), à **gauche ou droite** du texte (`iconRight`). Réglé par le segment ∅/icône + le bouton de côté de la ligne du calque.
- **Arrondi par calque** : chaque calque (sauf `logo`) a un `radius` 0..1 (slider « Arrondi » sous la ligne) ; défaut 0.56 pour les pastilles carrées (icône / logo techno seul), 1 pour les pilules.
