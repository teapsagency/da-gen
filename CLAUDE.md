...
- **Glyphe de pilule** : **rien** (défaut) ou une **icône/emoji custom** (mêmes champs `iconName`/`iconEmoji` que le calque icône, via le picker), à **gauche ou droite** du texte (`iconRight`, bouton ←/→ inline). Réglé par le segment ∅/icône de la ligne du calque. (Le champ `radius` existe sur `AssetLayer` mais sans contrôle UI pour l'instant — rendu avec ses défauts par type.)
