...
- **Layout** (`FrameSectorAsset`) : fond de card **transparent** → **export PNG transparent** (l'éditeur pose un fond clair dégradé derrière, juste pour la visibilité). Une **image flottante centrée** (taille = `imageScale`) avec **overlay bleu TEAPS** réglable (`veil`), entourée d'**éléments flottants** : icône (Lucide/emoji), logo TEAPS (forcé **noir**), pilule, badge, logo techno. **DA TEAPS figée** (`agencyLogo` + `TEAPS_ACCENT`).
- **Modèle d'élément** : `SectorAsset.elements` = `Partial<Record<AssetElementKey, {x,y}>>` (une clé présente = élément affiché ; `x/y` = centre 0..1). Déplacement en aperçu via une **poignée** (grip ; `onMoveElement` — pas de drag sur le corps, pour ne pas entrer en conflit avec le clic-image), ajout/retrait via le panneau ou le `×`. Positions par défaut : `DEFAULT_ELEMENT_POS` ; **agencement varié à chaque nouvelle illustration** via `LAYOUT_PRESETS` en rotation (`lib/sectorThemes.ts`). Migration des anciens assets (`slots`) → `migrateAssetShape` dans `loadAgencyAssets`.
- **Éditeur** (`SectorAssetEditor`) : aperçu à gauche + **panneau type Figma à droite** (Image de fond → popup, curseurs Taille/Voile, liste « Calques » avec + Ajouter / × Retirer + édition inline).
...
- **Logo de marque** (`BrandPickerModal`, élément flottant déplaçable parmi les autres) : set curé `simple-icons` (`lib/brandLogos.ts` → `BRAND_LOGOS`/`BRAND_MAP`, ~45 marques techno/CMS/e-commerce/marketing, couleur officielle de chaque marque). Stocké dans `SectorAsset.brandSlug`. Extensible : ajouter l'import `si…` + l'entrée dans `RAW`.
