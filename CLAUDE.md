...
- **Logo de marque** (`BrandPickerModal`, élément flottant déplaçable parmi les autres) : set curé `simple-icons` (`lib/brandLogos.ts` → `BRAND_LOGOS`/`BRAND_MAP`, ~45 marques techno/CMS/e-commerce/marketing, couleur officielle de chaque marque). Stocké sur le calque `brand` (`brandSlug`). Extensible : ajouter l'import `si…` + l'entrée dans `RAW`.
