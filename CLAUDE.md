...
- **Layout** (`FrameSectorAsset`) : fond **clair** (dégradé blanc→bleu très léger), une **image flottante centrée** (taille = `imageScale`, fraction de la card, avec ombre), entourée d'**éléments flottants déplaçables à la souris** : icône (Lucide/emoji), logo TEAPS (forcé **noir** sur le clair), pilule, badge, logo techno. **DA TEAPS figée** (`agencyLogo` + `TEAPS_ACCENT`).
- **Modèle d'élément** : `SectorAsset.elements` = `Partial<Record<AssetElementKey, {x,y}>>` (une clé présente = élément affiché ; `x/y` = centre 0..1). Drag en aperçu (`onMoveElement`), ajout/retrait via l'UI ou le `×`. Positions par défaut : `DEFAULT_ELEMENT_POS` (`lib/sectorThemes.ts`). Migration des anciens assets (`slots`/`veil`) → `migrateAssetShape`, appliquée dans `loadAgencyAssets`.
